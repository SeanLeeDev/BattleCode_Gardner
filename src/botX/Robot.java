...
abstract class Robot {
    ...
    static final int MAP_EDGE_BROADCASE_OFFSET = 10;
    ...
    public static void init(RobotController rc) throws GameActionException {
        Robot.rc = rc;
        random = new Random(System.getProperty("bc.testing.seed", "0").hashCode() + 1);
        type = rc.getType();
    ...
    // Gets called once at the start of the game for a single unit
    static void onStartOfGame() throws GameActionException {
        explorationOrigin = rc.getLocation().translate(-PATHFINDING_NODE_SIZE * PATHFINDING_WORLD_WIDTH / 2,
                -PATHFINDING_NODE_SIZE * PATHFINDING_WORLD_WIDTH / 2);
        broadcast(EXPLORATION_ORIGIN, explorationOrigin);

        rc.broadcastFloat(MAP_EDGE_BROADCASE_OFFSET + (0 + 1), mapEdges0);
        rc.broadcastFloat(MAP_EDGE_BROADCASE_OFFSET + (1 + 1), mapEdges1);
        rc.broadcastFloat(MAP_EDGE_BROADCASE_OFFSET + (2 + 1), mapEdges2);
        rc.broadcastFloat(MAP_EDGE_BROADCASE_OFFSET + (3+ 1), mapEdges3);

        // Reset some of the broadcast values
        rc.broadcast(GARDENER_OFFSET, -1000);
        rc.broadcast(HIGH_PRIORITY_TARGET_OFFSET, -1000);

        // Ensure we have counted the number of alive archons correctly
        ...
        for(int i = 0; i < initialArchonLocations.length; i++) {
            int locs = ARCHON_LOCATIONS + i * 3;
            rc.broadcastFloat(locs + 0, initialArchonLocations[i].x);
            rc.broadcastFloat(locs + 1, initialArchonLocations[i].y);
            rc.broadcast(locs + 2, -1);
        }
    }
    ...
    // A single invocation may take longers than one tick
    ...
    // Sometimes returns the wrong results, but if fixed the bot gets worse
    ...
    // @param id ID of the entity, ranges from 0 - 32000
    static void markAsHighPriority(int id) throws GameActionException {
        // NOTE that the right shift by id will only use the last 5 bits and thus will pick out the id'th bit in the broadcast
        // array when starting from the offset HIGH_PRIORITY
    ...
    static public void markEnemySpotted(RobotInfo[] units) throws GameActionException {
        if(!enemiesHaveBeenSpotted) {
            ...
            for(RobotInfo robot : units) {
                if(robot.team == enemy) {
                    if(robot.type == RobotType.SCOUT || robot.type == RobotType.ARCHON) count++;
                    else count += 3;
                }
            }
    ...
    // Attempt to move in a given direction, while avoiding small obstacles directly in the path.
    ...
    // Attempts to move in a given direction, while avoiding small obstacles in the path
    // @param dir               The intended direction of movement
    // @param degreeOffset      Spacing between checks directions(in degrees)
    // @param checksPerSide     Number of extra directions checked on each side, if intended direction was unavailable
    // @return                  true if a move was perform
    static boolean tryMove(Direction dir, float degreeOffset, int checksPerside, float distance) throws GameActionException {
        // First, try to move the intended direction
        if(rc.canMove(dir, distance)) {
            rc.move(dir, distance);
            return true;
        }
        // Now attempt to move in a bunch of similar angles
        int currentCheck = 1;
        while(currentCheck <= checksPerside) {
            ...
            //Try the offset on the right side next
        ...
        // A move never happened, return false
        return false;
    }

    //
    static void onStartOfTick() throws GameActionException {
        roundAsStart = rc.getRoundNum();

        if(rc.readBroadcast(PRIMARY_UNIT) != rc.getRoundNum()) {
            // We are the designated primary unit(first unit in spawn order(ARCHONS typically)
    ...
    static void yieldAndDoBackgroundTasks() throws GameActionException {
        ...
        if(rc.getRoundNum() != roundAsStart) {
            System.out.println("Error! Did not finish within the alloted bytecodes");
        }
    ...
    static MapLocation nextPointOnPathToEnemyArchon(MapLocation loc) throws GameActionException {
        ...
        // Direction needs to be reversed to move towards the archon
        dir = (dir + 2) % 4;
    ...
    // Returns a bitpacked value where bit 0 indeictes if the node is blocked and bit 1 indicates if the node chunk has been explored
    ...
    static void broadcastExploration() throws GameActionException {
        ...
        for(int dy = -1; dy <= 1; dy++) {
            ...
            for(int dx = -1; dx <= 1; dx++) {
                ...
                boolean outdate = ((chunkInfo >> 21) & 0xF) != recalculationTime;
                if((chunkInfo & (1 << 31)) == 0 || outdate) {
                    // This chunk is still unexplored
                    MapLocation chunkCenter = explorationOrigin.translate((nx + 0.5f) * PATHFINDING_CHUNK_SIZE
                            * PATHFINDING_NODE_SIZE, (ny + 0.5f) * PATHFINDING_CHUNK_SIZE * PATHFINDING_NODE_SIZE);
                    ...
                    if(rc.canSensePartOfCircle(chunkCenter, PATHFINDING_CHUNK_RADIUS)) {
                        ...
                        // Reservoir Sampling
                        totalWeight += weight;
                        if(random.nextFloat() * totalWeight < weight) {
                            // Start a new job
                            ...
                            jobChunkNy = ny;
                            jobChunkNodeIndex = 0;
                            // Clear the recalculation time for the cunk and update it with the current time
                            jobChunkInfo = (chunkInfo & (~(0xF << 21))) | recalculationTime << 21;
                            jobNodeSkips = 0;
                            jobChunkWasOutdate = outdate;
                        }
                    }
                }
            }
    ...
    static private boolean jobChunkWasOutdate;
    ...
    static boolean doChunkJob() throws GameActionException {
        ...
        // Do not inspect, chunk has been fully explored
        boolean alreadyFullyExplored = (jobChunkInfo & (1 << 31)) != 0;

        // Loop through all of the nodes that are contained within the chunk
        for(; jobChunkNodeIndex < PATHFINDING_CHUNK_SIZE * PATHFINDING_CHUNK_SIZE; jobChunkNodeIndex++) {
            if(Clock.getBytecodesLeft() < 500 || Clock.getBytecodeNum() - startingTime > 6000)
                return false;

            int prevValue = jobChunkInfo & (1 << jobChunkNodeIndex);
            if(prevValue != 0) {
                if(!jobChunkWasOutdate) {
                    // Already figured out that this is not traversable in an earlier update
                    // wasn't very long ago, assume that the world looks the same
                    continue;
                }
            }
            ...
            MapLocation nodeCenter = explorationOrigin.translate(((x + 0.5f) + jobChunkNx * PATHFINDING_CHUNK_SIZE)
                    * PATHFINDING_NODE_SIZE, ((y + 0.5f) + jobChunkNy * PATHFINDING_CHUNK_SIZE) * PATHFINDING_NODE_SIZE);
            if(!onMap(nodeCenter, PATHFINDING_NODE_SIZE * 0.5f)) {
                traversable = false;
            }
            else {
                if(!rc.canSenseAllOfCircle(nodeCenter, PATHFINDING_NODE_SIZE)) {
                    // Skip this one. If we have previously explored the whole chunk then we just want to update it
                    // with as much new information as we can, don't count skipped nodes because they will not contain last info
                ...
                if(rc.isCircleOccupiedExceptByThisRobot(nodeCenter, PATHFINDING_NODE_SIZE * 0.5f) && rc.senseNearbyTrees(nodeCenter,
                        PATHFINDING_NODE_SIZE * 0.5f, null).length > 0) {
                    traversable = false;
                }
            }
        ...
        if(jobNodeSkips == 0) {
            ...
            // If we have more or the same amount of info as the last time
            // this chunk was generated then update it, otherwise don't touch it
    ...
    // This returns true if the lcation is on the map, using the information collected so far
    static boolean onMap(MapLocation pos) {
        return pos.x <= mapEdges0 && pos.y <= mapEdges1 && pos.x >= mapEdges2 && pos.y >= mapEdges3;
    }

    // This returns true if the location is at least margin units from the edge of the map using current info
    ...
    static void shakeNearbyTrees() throws GameActionException {
        if(rc.canShake()) {
            TreeInfo[] trees = rc.senseNearbyTrees(type.bodyRadius + GameConstants.INTERACTION_DIST_FROM_EDGE - 0.00f);
            TreeInfo bestTree = null;
            for(TreeInfo tree : trees) {
                // Make sure it is not the tree of an opponent first
                if(tree.team == Team.NEUTRAL || tree.team ==ally) {
                    // Check tree for bullets and pick the tree with the most bullets in it
    ...
    static void determineMapSize() throws GameActionException {
        ...
        int globalMapEdgesDetermined = rc.readBroadcast(MAP_EDGE_BROADCASE_OFFSET);
        if(globalMapEdgesDetermined != mapEdgesDetermined) {
            mapEdgesDetermined = globalMapEdgesDetermined;
            mapEdges0 = rc.readBroadcastFloat(MAP_EDGE_BROADCASE_OFFSET + (0 + 1));
            mapEdges1 = rc.readBroadcastFloat(MAP_EDGE_BROADCASE_OFFSET + (1 +1));
            mapEdges2 = rc.readBroadcastFloat(MAP_EDGE_BROADCASE_OFFSET + (2 + 1));
            mapEdges3 = rc.readBroadcastFloat(MAP_EDGE_BROADCASE_OFFSET + (3 + 1));
        }
        ...
        for(int i = 0; i < 4; i++) {
            if((mapEdgesDetermined & (1 << i)) == 0) {
                ...
                if(!rc.onTheMap(rc.getLocation().add(angle, type.sensorRadius * 0.99f))) {
                    ...
                    rc.broadcastFloat(MAP_EDGE_BROADCASE_OFFSET + i + 1, result);
                    // This robot will pick up the change for real the next time determineMapSize is called
                    tmpDetermined |= (1 << i);
                    rc.broadcast(MAP_EDGE_BROADCASE_OFFSET, tmpDetermined);
                    ...
                    int otherEdgeIndex = MAP_EDGE_BROADCASE_OFFSET + ((i + 2) % 4) + 1;

                    // Make sure we DO NOT overwrite the edge with something worse
                    ...
                    rc.setIndicatorLine(mapEdge.add(angle + (float)Math.PI * 0.5f, 50),
                            mapEdge.add(angle - (float)Math.PI * 0.5f, 50), 255, 255, 255);
                }
            }
        }
    }
    ...
    static FirePlan fireAtNearbyRobot(RobotInfo[] friendlyRobots, RobotInfo[] hostileRobots, boolean targetArchons) throws GameActionException {
        ...
        for(RobotInfo robot : hostileRobots) {
            ...
            // Give higher priority to robots that do not seem to move
            if(robot.location.isWithinDistance(prevLoc, 0.3f)) {
                score *= 2;
            }

            score /= 4 + robot.health / robot.type.maxHealth;
            score /= myLocation.distanceTo(robot.location);
            if(score > bestScore) {
                bestScore = score;
                bestRobot = robot;
            }
            ...
            if(dist < 2 * type.sensorRadius && teamOf(firstUnitHit) == enemy) {
                ...
                if(rc.canFirePentadShot()) {
                    if(rc.getTeamBullets() > 300 && friendlyRobots.length < hostileRobots.length && (friendlyRobots.length == 0 || hostileRobots.length >= 2)) {
                        // ...Then fire a bullet in the direction of the enemy unit
                ...
                // And we have enough bullets, and haven't attacked this turn..
                if(rc.canFireSingleShot()) {
                    // Then fire a bullet in the direction of the given enemy
    ...
    // Returns the bullets/tick for a given tree
    ...
    static Team teamOf(BodyInfo body) {
        if(body != null && body.isRobot()) return ((RobotInfo)body).team;
        if(body != null && body.isTree()) return ((TreeInfo)body).team;
        return Team.NEUTRAL;
    }

    // Distance to the first tree. Makes use of sampling so it is not perfectly accurate
    // This returns a very large number if no tree was struck during the attack
    ...
    // Distance to first body on the line segment going from the edge3 of this robot to the specified location.
    // Uses sampling so it is not perfectly accurate. Also uses radius of robot for collision detection.
    static float linecastDistance(MapLocation body) throws GameActionException {
        MapLocation a = rc.getLocation();
        Direction dir = a.directionTo(body);
        float dist = a.distanceTo(body);
    ...
    // First body on the line segment going from the edge of this robot to the specified location. Uses sampling/inaccurate
    static BodyInfo linecast(MapLocation body) throws GameActionException {
        MapLocation a = rc.getLocation();
        Direction dir = a.directionTo(body);
        float dist = a.distanceTo(body);
    ...
    static void distBug(MapLocation target) throws GameActionException {
        ...
        float distanceToTarget = rc.getLocation().distanceSquaredTo(target);

        // Already at the current target
        ...
        if(rc.canMove(rc.getLocation().directionTo(target))) {
            ...
            if(distanceToTarget - hitDistance < historicalMinDist - step) {
                // Move directly to the selected target
        ...
        for(int tries = 0; tries < 2; tries++) {
            ...
            float dtheta = tries == 0 ? 6 : 1f + random.nextFloat()*0.1f;
            float stride = tries == 0 ? type.strideRadius : type.strideRadius * random.nextFloat();
            ...
            if(rc.canMove(lastBugDir, stride) && bugTieBreaker!= 0) {
                ...
                if(!any) {
                    ...
                    if(bugTieBreaker == 0) {
                        ...
                        if(Math.abs(tieBreakerDirection.rotateRightDegrees(90).degreesBetween(lastBugDir)) > 90 && random.nextFloat() < 0.7f) {
                            // Rotate right
                            bugTieBreaker = 1;
                        }
                        else {
                ...
                // If we can move in the new direction and(we have tried really hard or it is a small change in dir)
    ...
    static MapLocation moveToAvoidBullets(MapLocation secondaryTarget, BulletInfo[] bullets, RobotInfo[] units) throws GameActionException {
        ...
        if(type != RobotType.GARDENER) {
            ...
            MapLocation randomPos = myLocation.add(random.nextFloat() * (float)Math.PI * 2, random.nextFloat() * (type.bodyRadius + PATHFINDING_NODE_SIZE * 0.5f));

            int index = snapToNode(randomPos);
        ...
        // THe first bulletsToConsider bullets can potentially hit us after the loop is done
        ...
        // Primary bullet avoidance loop. Tests various driections to move in and picks
        // the "best" one for somet definition of best
        MapLocation bestMove = null;
        int iterationsDone = 0;
        // Save some processing power if we have no bullets to consider(will be used by exploration code)
        ...
        while((Clock.getBytecodesLeft() > 3000 && iterationsDone < maxIterations || iterationsDone < 2)) {
            ...
            iterationsDone += 1;
            if(rc.canMove(loc)) {
                // First check the estimated bullet damage and only if that is the same for two
                // directions, then break ties using getSecondaryMovementScore
    ...
    static float getSecondaryMovementScore(MapLocation loc, MapLocation reservedNodeLoc, RobotInfo[] units, MapLocation target) {
        ...
        if(type == RobotType.LUMBERJACK) {
            // Sense nearby trees but re-use the cache if we already sense them during the tick
            if(treeCacheRound != rc.getRoundNum()) {
                treeCacheRound = rc.getRoundNum();
                treeCache = rc.senseNearbyTrees();
            }
                ...
                if(unit.team == myTeam) {
                    ...
                    switch (unit.type) {
                        ...
                        default:    // Scout/Soldier/Tank
                            // NOTE: Potential should be positive for some point within the sensor radius otherwise we flee
                            if(isCoward)
                                score -= 2f / (loc.distanceSquaredTo(unit.location));
                            else
                                score += 1f / (loc.distanceSquaredTo(unit.location));
                            break;
                        case LUMBERJACK:
                            float dis = loc.distanceTo(unit.location);
                            // NOTE potential should be positive for some point within the sensor radius otherwise flee
                            score -= 10f / (dis * dis + 1);
    ...
    static float getEstimatedDamageAtPosition(float locx, float locy, int numBullets, float[] bulletX, float[] bulletY, float[] bulletDx, float[] bulletDy, float[] bulletDamage, float[] bulletSpeed, float[] bulletImpactDistances) {
        float dmg = 0f;
        float radius = type.bodyRadius;
        float sqrRadius = radius * radius;
        ...
        for(int i = 0; i < numBullets; i++) {
            ...
            if(bulletImpactDistances != null && intersectionDist1 > bulletImpactDistances[i]) {
                // Bullet has already hit a tree or another unit
                continue;
            }

            // -1 because the bullet has not move this frame yet
            float timeToIntersection = (intersectionDist1 / bulletSpeed[i]) - 1;
            // If will hit us this frame
    ...
    static <T> T randomChoice(T[] values, T def) {
        return values.length > 0 ? values[random.nextInt(values.length)] :def;
    }
    ...
    static MapLocation pickRandomSpreadOutTarget(MapLocation[] cands, int index) throws GameActionException {
        int r = random.nextInt(cands.length);
        if(r + 1 == rc.readBroadcast(RANDOM_LAST + index)) {
            r = random.nextInt(cands.length);
        }
    ...
    static MapLocation pickRandomCloseTarget(MapLocation[] cands) {
        ...
        for(MapLocation cand : cands) {
            float p = 0.1f + 1f / (1f + rc.getLocation().distanceSquaredTo(cand));
            sump += p;
            if(p / sump <= random.nextFloat()) {
                chosen = cand;
            }
        }
        return chosen;
    ...
    static void considerDonating() throws GameActionException {
        ...
        // We only have a single scout that is alive. Strategy should be to buy as many VPs
        // that we can and try to stay alive until the end of the game
        final boolean onlySingleScoutAlive = type == RobotType.SCOUT && rc.getRobotCount() == 1;

        int toKeep = 0;
        float turnsUntilVictory = 0f;

        // Income depends on the number of bullets we decide to keep which depends on the
        // income so iterate a few times to make sure it converges
        ...
        for(TreeInfo tree : trees) {
            ...
            if(tree.getTeam() == enemy) {
                ...
                if(firstUnitHit != null && firstUnitHit.isTree() && ((TreeInfo)firstUnitHit).getTeam() == enemy) {
                    ...
                    if(rc.canFireSingleShot() && turnsLeft > STOP_SPENDING_AT_TIME && t.getHealth() > 10 && (t.getHealth() < 45 ||t.location.distanceTo(rc.getLocation()) < 3f)){
