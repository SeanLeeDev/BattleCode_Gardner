...
public class Archon extends Robot {
    ...
    public boolean trySacrifice() throws GameActionException {
        // If the game has been running for a while and no archon seems to have been able to build a gardener
        ...
        if(rc.getRoundNum() > 50 && !everBuiltAGardener && spawnedCount(RobotType.GARDENER) == 0 && rc.getRobotCount() <= ourInitialArchonLocations.length) {
            for(int attempts = 0; attempts < 40; attempts++) {
                ...
                if(rc.canHireGardener(dir)) {
                    System.out.println("ARCHON SACRIFICED!");
                    rc.hireGardener(dir);
                    int gardenerCount = spawnedCount(RobotType.GARDENER);
            ...
            if(rc.getRoundNum() > 70) {
                ...
                if(nearbyArchons > 0 && rc.getRoundNum() > 70 + ourInitialArchonLocations.length - nearbyArchons) {
                    // Still haven't been able to build a gardener, but there are ally archons nearby.
                    // Sacrifice this archon and hope the others will be able to build something.
                    // Start with archon that has the most ally archons close to it
    ...
    private static final CustomQueue tertiarryQueue = new CustomQueue();
    ...
    void addPathfindingseed(MapLocation seed) {
        if(explored == null) {
            ...
            // us from going out of bounds (they will never be traversable anyways)
    ...
    // A highly optimized pathfinding routing, implements a BFS
    void pathFinding() throws  GameActionException {
        ...
        while (true) {
            ...
            else if(tertiarryQueue.head != tertiarryQueue.tail) {
                node = tertiarryQueue.pollFirst();
            }
            else {
                break;
            }
            ...
            for(int i = 0; i < 4; i++) {
                ...
                if(explored[nindex] < pathfindingIndex) {
                    ...
                    // Traversable if this is == 0, block if it is == 1
                    ...
                    switch (blocked | fullyExplored) {
                        ...
                        // Traversable, but not fully explored. It might actually be blocked upon attempting
                        default:
                            ...
                            searchTime3 += Clock.getBytecodeNum();
                            break;
                    }
                }
            }
    ...
    void broadcastPathfindingResult() throws GameActionException {
        ...
        for(int cy = 0; cy < PATHFINDING_WORLD_WIDTH / PATHFINDING_CHUNK_SIZE; cy++) {
            ...
            // DO NOT bother broadcasting information for tiles outside the map
            if(!onMapY(wy, -PATHFINDING_CHUNK_SIZE * PATHFINDING_NODE_SIZE * 0.5f)) continue;

            for(int cx = 0; cx < PATHFINDING_WORLD_WIDTH / PATHFINDING_NODE_SIZE; cx++) {
