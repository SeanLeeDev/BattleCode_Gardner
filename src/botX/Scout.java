...
public class Scout extends Robot {
    ...
    private boolean highPriorityTargetExists() throws GameActionException {
        ...
        return rc.getRoundNum() < lastAttackingEnemySpotted + 20 || rc.getRoundNum() < lastAttackingEnemySpotted + 20;
    }
    ...
    //
    private float getPositionScore(MapLocation loc, RobotInfo[] units, int numBullets, float[] bulletX, float[] bulletY, float[] bulletDx,
                                   float[] bulletDy, float[] bulletDamage, float[] bulletSpeed, float[] bulletImpactDistances, TreeInfo bestTree, MapLocation target) {
        float score = 0f;
            ...
            if(unit.team == ally) {
                ...
                switch (unit.type) {
                    ...
                    case ARCHON:
                        break;
                    default:
                        float dis2 = loc.distanceSquaredTo(unit.location);
        ...
        if(bestTree != null) {
            score += (bestTree.containedBullets * 0.5) / (loc.distanceSquaredTo(bestTree.location));
        }
    ...
    private TreeInfo findBestTreeToShake() throws GameActionException {
        ...
        for(int i = 0; i < numTrees; i++) {
            ...
            if(tree.containedBullets > 0) {
                float score = tree.containedBullets / (1 + myLocation.distanceSquaredTo(tree.location));
    ...
    public void onUpdate() throws GameActionException {
        ...
        RobotInfo[] friendlyUnits = rc.senseNearbyRobots(-1, ally);
        ...
        for(int i = 0; i < bulletsToConsider; i++) {
            if(!bulletCanHitUs(myLocation, nearbyBullets[i])){
                ...
                bulletDy[i] = bullet.dir.getDeltaX(1);
        ...
        if(random.nextInt(200) < 1 || myLocation.distanceSquaredTo(target) < 4f) {
            target = pickTarget();
        }
        ...
        // Save some processing power if we have no bullets to considering(will be used by exploration code)
        ...
        while((Clock.getBytecodesLeft() - processingTime > 3000 && iterationsDone < maxIterations) || iterationsDone < 3) {
            ...
            if(rc.canMove(loc)) {
                ...
                float score = getPositionScore(loc, allRobots, bulletsToConsider, bulletX, bulletY, bulletDx,
                        bulletDy, bulletDamage, bulletSpeed, bulletImpactDistances, bestTree, target);
                if(score > bestScore) {
                    bestScore = score;
                    bestMove = loc;
                }
        ...
        FirePlan firePlan = fireAtNearbyRobot(friendlyUnits, robots, targetArchons);
        if(firePlan != null){
            firePlan.apply(rc);
        }
