...
public class Tank extends Robot {
    ...
    static MapLocation pickTarget(MapLocation[] fallBackPositions, float pickFallbackProbability) throws GameActionException {
        ...
        if (random.nextFloat() < pickFallbackProbability) {
            if (isDefender) {
    ...
    static MapLocation pathfindingTarget() throws GameActionException {
        ...
        // Try to take a shorter path if possible
        if (linecast(c) == null) {
            return c;
        } else {
    ...
    // Exponentially decaying weighted average of the speed the unit has toward the target.
    // If this is very low (or negative) then the unit is not making much progress and maybe
    // a different target should be chosen
    ...
    static void fireAtNearbyNeutralTree(TreeInfo[] trees) throws GameActionException {
        if (rc.getTeamBullets() < 100 || rc.getRoundLimit() - rc.getRoundNum() <= STOP_SPENDING_AT_TIME || trees.length == 0)
            return;
    ...
    public void onUpdate() throws GameActionException {
            ...
            if (moveTo == null) {
                ...
                if (deg > 90) {
                    ...
                    if (firePlan == null) {
                        ...
                        // We are moving in the direction we are firing in, so to avoid moving on top of our own bullets
                        // we should move first and then shoot after
                        rc.move(moveTo);
                        firePlan = fireAtNearbyRobot(friendlyUnits, robots, targetArchons);
                        if (firePlan != null) {
                            firePlan.apply(rc);
                        }
                    }
        ...
        if (!rc.hasAttacked()) {
            ...
            if (nearbyTrees / (nearbyGardeners + 0.01) > 15)
                fireAtNearbyNeutralTree(enemyTrees);
        }
